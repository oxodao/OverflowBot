<template>
  <div id="AddCommand" @click="hideFrame" ref="container">
    <form v-on:submit.prevent="submit">
      ...
      <div>
        ...
        <label for="desc">Text répondu:</label>
        <textarea id="desc" rows="5" v-model="resp" required :disabled="state === 'UPLOADING'"/>
...
export default {
  ...
  methods: {
    submit() {
      ...
      axios.post('/api/command', fd, { headers: { "Content-Type": "multipart/form-data", "Authorization": this.$store.state.User.token } })
        .then((e) => {
